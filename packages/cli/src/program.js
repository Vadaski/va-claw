...
function createCliProgram(deps = createDefaultCliDeps()) {
  ...
  sessionAppend.action(async (options) => runSessionAppend(options, deps));
  ...
  sessionRecall.action(async (options) => runSessionRecall(options, deps));
  ...
  lark.command("setup").description("Configure Lark (Feishu) bot credentials.").option("--app-id <id>", "Lark app ID").option("--app-secret <secret>", "Lark app secret").option("--cli-command <command>", "CLI command to invoke for each message").option("--notify-chat-id <id>", "Lark chat ID to receive wake notifications").action(
    async (options) => runLarkSetup(options.appId, options.appSecret, options.cliCommand, options.notifyChatId, deps)
  );
